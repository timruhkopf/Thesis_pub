    ...
    \section{A note on Bayesian posterior inference} \label{inference}
      ...
      %  The detailed balance condition in \cite{neal2011mcmc} chp. 3.2 sheds light on the connection of HMC to HM:
       ...
       %    \frac{V}{Z}exp(-H_{A_k})min\left[1,exp(-H_{B_k} + H_{A_k})\right] = \frac{V}{Z}exp(-H_{B_k})min\left[1,exp(-H_{A_k} + H_{B_k})\right]
        ...
        \cite{neal2011mcmc} shows that Hamiltonian dynamics are reversible, easing the proof of Bayesian updates using Hamiltonian dynamics leaving the desired distribution invariant. The proof requires the Markov chain transitions, which in case of HMC are generated by Hamiltonian dynamics, to be reversible.

        % what is the role of volume for Bayesian? - no adjustment of acceptance rate to keep it a proper transition?
