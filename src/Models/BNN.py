...
class BNN(nn.Module, Util_Model):
    ...
    def reset_parameters(self, separated=False):
        ...
        :param separated: bool. indicates whether the first layer (given its
        local reset_parameters function has a separated argument) will sample
        ...
        if 'separated' in inspected:
            self.layers[0].reset_parameters(separated)
        else:
            # default case: all Hidden units, no shrinkage -- has no separated attr.
