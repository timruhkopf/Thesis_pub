...
if __name__ == '__main__':
    ...
    sbnn = ShrinkageBNN(hunits=[no_in, 10, 5, no_out], shrinkage='glasso', bijected=True, seperated=True)
    ...
    path = '/home/tim/PycharmProjects/Thesis/Experiment/Shrinkage_glasso/'
    counter = 0
    while True:
        counter += 1
        ...
        try:
            ...
            sbnn = ShrinkageBNN(hunits=[no_in, 2, 2, no_out], shrinkage='glasso', bijected=True, seperated=True)
            sbnn.reset_parameters(seperated=True)
            sbnn.true_model = deepcopy(sbnn.state_dict())
            y = sbnn.likelihood(X).sample()
        ...
        try:
            ...
            config = {'n': 1000, 'n_val': 100, 'model_class': ShrinkageBNN, 'model_param':
                dict(hunits=[no_in, 10, 5, no_out], shrinkage='multilasso', bijected=True, seperated=True),
                      'sampler_name': 'RHMC', 'sampler_param': dict(epsilon=0.001, L=1), 'seperated': True}
